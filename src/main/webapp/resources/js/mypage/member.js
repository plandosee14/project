$(function() {
   ...
   /*
    //비밀번호 변경 화면 : 비빌번호 확인 버튼 클릭 이벤트 원본코드
   $("#pass_check_btn").click(function(){
      var check_password = $("#check_password").val();
   ...
   */
   ...
   $("#pass_edit_btn").click(function(){
      var member_mail = $("#session_mail").val();
      var member_pass = $("#check_password").val();
      var new_pass = $("#member_pass").val();
      
      // String login_mail, String login_pass, String new_pass
      $.ajax({
         url : '/dreamUp/password_edit.do',
         type : 'post',
         data : {
            "login_mail" : member_mail,
            "login_pass" : member_pass,
            "new_pass" : new_pass
         },
         success : function(editPass) {
            if(editPass=='success'){
               ...
               alert("비밀번호 변경에 실패하였습니다. 처음부터 다시 진행해주세요.");
            }
         }// success
      });//ajax
